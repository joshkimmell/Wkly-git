...
export default function AccomplishmentsModal({ goalTitle, isOpen, onClose, accomplishments, onCreate, onDelete, onEdit, loading }: Props) {
  ...
  const handleAdd = async () => {
    ...
    await onCreate({ title: title.trim(), description: description.trim(), impact: impact.trim() || undefined });
    setTitle('');
    setDescription('');
    setImpact('');
  };
      ...
      <div className={`${modalClasses} w-full max-w-2xl`}>
        ...
        <div className="space-y-4 max-h-[60vh] overflow-y-auto">
          <div>
            ...
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Description" rows={3} className="mt-1 block w-full border-gray-30 focus:border-b-2 focus:ring-0" />
                ...
                <li key={acc.id} className="p-3 border rounded bg-gray-10 dark:bg-gray-80 dark:border-gray-70">
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-gray-60 dark:text-gray-40 mt-1"><span dangerouslySetInnerHTML={{ __html: acc.description }} /></div>
                      {acc.impact?.trim() && (
                    ...
                    <div className="flex items-center gap-2 ml-4">
                      ...
                      <button className="btn-ghost" onClick={() => onDelete(acc.id)} title="Delete accomplishment"><Trash className="w-4 h-4" /></button>
                    </div>
                  </div>
                </li>
              ))}
