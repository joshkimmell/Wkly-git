...
const SummaryCard: React.FC<SummaryCardProps> = ({ summary, handleDelete, handleEdit }) => {
  ...
  return (
    <div className="flex flex-col text-left w-full space-y-4">
      <div key={summary.id} className="flex flex-col items-left w-full space-y-2 bg-white shadow-sm border rounded-lg p-4">
        <h4 className="text-lg font-bold text-gray-900">{summary.title}</h4>
        <button
          ...
          {!isExpanded ? (
            // <ChevronUp className="w-5 h-5" />
              <div className='flex flex-col text-left'>
                <ReactMarkdown>{summary.content.substring(0, 200) + (summary.content.length > 200 ? '...' : '')}</ReactMarkdown>
                {/* <ChevronUp className="w-5 h-5" /> */}
              </div>
            ) : (
              <div className='flex flex-col text-left'>
